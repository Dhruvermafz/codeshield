...
const PasswordItem = ({ index, item, onUpdate }: Props) => {
  ...
  return (
    <div
      className={`cursor-pointer w-[23rem] mx-auto transition-all border border-gray-600 duration-300 ease-in-out md:w-96 my-3 rounded-xl shadow-lg pt-3 px-1 ${
        isExpanded
          ? `h-auto ${
              theme === "light" ? "bg-[#F8F7F4]" : "bg-[black]"
            } border border-gray-500`
          : `h-16 ${theme === "light" ? "bg-[white]" : "bg-[#1b1b1b]"}`
      }`}
    >
      <div
        onClick={toggleExpand}
        className="flex p-0 justify-between items-center"
      >
        <div className="flex px-4 justify-start items-center">
          <SiteIcon />
          <h2
            className={`text- mx-2 font-medium ${
        ...
        <p
          className={`${theme === "light" ? "text-black" : "text-white"} mx-5`}
        >
          {isExpanded ? "▲" : "▼"}
        </p>
      </div>
        ...
        <div className="p-6 pt-5">
          <div
            className={`flex justify-between items-center ${
              theme === "dark" ? "text-[#CECECE]" : "text-[#252525]"
            }`}
          >
          ...
          >
            ...
            <PasswordStrengthIndicator
              password={decryptPassword(item.password, item.iv)}
            />
          </div>
          ...
          <div className="mt-3 flex justify-between items-center">
            <button
              ...
              className={`px-3 w-28 py-2 mr-1 ${
                theme === "dark"
                  ? "bg-[#292929] text-white"
                  : "bg-[#F8F8FF] text-[black]"
              } border border-gray-500 rounded-lg hover:bg-violet-500`}
            >
              Edit
            </button>
            <button
              onClick={() => setShowPopUp({ type: "delete", status: true })}
              className={`px-3 w-28 py-2 mx-1 ${
                theme === "dark"
                  ? "bg-[#292929] text-[#E74C3C]"
                  : "bg-[#F8F8FF] text-[#E74C3C]"
              } border border-gray-500 rounded-lg hover:text-white  hover:bg-[#E74C3C]`}
            >
              Delete
            </button>
